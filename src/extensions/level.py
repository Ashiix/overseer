...
async def level(self, message, user):
    with SqliteDict("./db/level.sqlite", autocommit=True) as level_db, SqliteDict(
        "./db/notif.sqlite", autocommit=True
    ) as notif_db:
        ...
        if level_db[user.id] % config.level_curve == 0:
            if (
                notif_db.get(str(user.id) + "_level") == None
                or notif_db[str(user.id) + "_level"] == 1
            ):
                await self.get_channel(config.level_channel).send(
                    "woah, "
                    + user.mention
                    + ", you're now level "
                    + str(get_level(user))
    ...
    with SqliteDict("./db/level.sqlite", autocommit=True) as level_db:
        ...
        for i in range(min(10, len(level_db))):
            ...
            leaderboard += (
                ...
                + ": "
                + str(
                    parse_level(sorted_levels.get(sorted_keys[len(level_db) - i - 1]))
                )
                + "\n"
            )
        leaderboard += (
...
async def toggle_notif(self, user):
    with SqliteDict("./db/notif.sqlite", autocommit=True) as notif_db:
        if (
            notif_db.get(str(user.id) + "_level") == None
            or notif_db[str(user.id) + "_level"] == 0
        ):
